...
for( my $i = 0; $i <= @ip_lst; $i++){
	...
	###	if distro is VMWARE,
	if( does_It_Have($this_distro, "vmware") ){

		###	ADDED 022912	do not reboot esxi-40
#		if( $this_version eq "esxi-4.0" ){			### TAKEN OUT 	040512
#			print "\nESXI-4.0 was detected\n";
#			print "Reboot is not needed\n";
#		}elsif( $this_version eq "esxi-4.1" ){
#			print "\nESXI-4.1 was detected\n";
#			print "Reboot is in 2 steps\n";
#			reboot_machine_in_2steps($this_ip);
#			print "\n";
#			$is_rebooted = 1;
#		}else{
...
if( $is_error == 1 ){
	print "\n[TEST_REPORT]\tFAILED TO REBOOT VMWARE MACHINES !!!\n\n";
	exit(1);
};
...
if( $is_rebooted == 1 ){
	...
	sleep(300);
	print "\n[TEST_REPORT]\tVMWARE REBOOT IS FINISHED\n\n";

}else{
	print "\n[TEST_REPORT]\tVMWARE REBOOT WAS UNNECESSARY\n\n";
};
...
sub reboot_machine{
	...
	print("ssh -o BatchMode=yes -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$this_ip \"shutdown.sh; sleep 5; reboot -f\"\n");
	print "\n";

	$outstr = `ssh -o BatchMode=yes -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$this_ip \"shutdown.sh; sleep 5; reboot -f\"`;
...
sub reboot_machine_in_2steps{
	...
	print("ssh -o BatchMode=yes -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$this_ip \"shutdown.sh\"\n");
	print "\n";

	$outstr = `ssh -o BatchMode=yes -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$this_ip \"shutdown.sh\"`;
